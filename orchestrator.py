...
app = FastAPI(title="Unified AI Agent Orchestrator", version="1.0.0") if _FASTAPI_AVAILABLE else None
...
if app is not None:
    ...
    async def startup_event():
        ...
        asyncio.create_task(_orchestrator.start(num_workers=config.MAX_CONCURRENT_JOBS))
...
def _register_routes(app, _orchestrator):
    """Register all FastAPI routes (called only when FastAPI is available)."""
    ...
    async def root():
        ...
        return {
            ...
            "repositories": len(_orchestrator.repo_manager.list_repos()),
            "status": "/status",
            "docs": "/docs"
        }
    ...
    async def create_task(task: Dict[str, str]):
        ...
        task_id = await _orchestrator.submit_task(task_text, agent_name)
    ...
    async def list_skills(category: str = None, search: str = None):
        ...
        agent = _orchestrator.agents.get("default")
        if not agent:
            return {"skills": []}
    ...
    async def clone_repository(repo_name: str, background_tasks: BackgroundTasks):
        ...
        background_tasks.add_task(
            _orchestrator.repo_manager.clone_repo,
            repo_name,
            repo_info["url"]
        )
    ...
    async def analyze_income_opportunities(platforms: List[str] = None):
        ...
        agent = _orchestrator.agents.get("content_agent") or _orchestrator.agents.get("default")
    ...
    async def scan_prediction_markets(market_type: str = "arbitrage"):
        ...
        agent = _orchestrator.agents.get("finance_agent") or _orchestrator.agents.get("default")
...
if app is not None:
    _register_routes(app, _orchestrator)
